    ...
    <Link to={`/products/${product.slug}`} className="group block relative">
      ...
      >
        ...
        <div className="absolute top-3 left-3 flex flex-col gap-2">
          ...
          {product.sourcePriceUsdCents && <Badge variant="sale">SALE</Badge>}
        </div>
      ...
      <div className="space-y-1 px-1">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs font-medium">{product.ratingAvg?.toFixed(1) || '0.0'}</span>
          <span className="text-xs text-textSecondary">({product.ratingCount || 0})</span>
        </div>
